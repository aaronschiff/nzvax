...
dat_sa2 <-  read_excel(path = here(glue("data/covid_vaccinations_{latest_date}.xlsx")), 
  ...
  mutate(fully_vax_category = case_when(
    dose2_uptake > 900 ~ "Greater than 90%", 
    (dose2_uptake > 800) & (dose2_uptake <= 900) ~ "80% to 90%", 
    (dose2_uptake >= 700) & (dose2_uptake <= 800) ~ "70% to 80%", 
    TRUE ~ "Less than 70%"
  )) |> 
  mutate(first_dose_category = case_when(
    dose1_uptake > 900 ~ "Greater than 90%", 
    (dose1_uptake > 800) & (dose1_uptake <= 900) ~ "80% to 90%", 
    (dose1_uptake >= 700) & (dose1_uptake <= 800) ~ "70% to 80%", 
    TRUE ~ "Less than 70%"
  ))
