...
# First doses age 12+
chart_first_doses_12plus <- dat_chart |> 
  filter(age == "12+") |> 
  ...
  ggtitle(label = "Age 12+ NZ COVID-19 vaccination rates for each suburb (SA2 area)", 
...
ggsave(filename = here(glue("outputs/latest/first_doses_SA2_12plus_{latest_date}.png")), 
       plot = chart_first_doses_12plus, 
       device = agg_png, 
       width = 2600, 
       height = 1800, 
       units = "px", 
...
# First doses age 5-11
chart_first_doses_5_11 <- dat_chart |> 
  filter(age == "5-11") |> 
  ggplot(mapping = aes(x = dose1_uptake / 100, 
                       y = 0, 
                       fill = first_dose_category)) + 
  ...
  ggtitle(label = "Age 5-11 NZ COVID-19 vaccination rates for each suburb (SA2 area)", 
          subtitle = glue("People who have received at least one dose to {latest_date_nice} (percent of age 5-11 population)")) + 
  theme_minimal(base_family = "Fira Sans Custom") + 
  theme(panel.grid.major.y = element_blank(), 
        panel.grid.minor = element_blank(), 
        panel.grid.major.x = element_line(size = 0.25), 
...
ggsave(filename = here(glue("outputs/latest/first_doses_SA2_5_11_{latest_date}.png")), 
       plot = chart_first_doses_5_11, 
       device = agg_png, 
       width = 2600, 
       height = 1800, 
       units = "px", 
...
# Second doses age 12+
chart_fully_vax_12plus <- dat_chart |> 
  filter(age == "12+") |> 
...
ggsave(filename = here(glue("outputs/latest/fully_vax_SA2_12plus_{latest_date}.png")), 
       plot = chart_fully_vax_12plus, 
       device = agg_png, 
       width = 2600, 
       height = 1800, 
       units = "px", 
